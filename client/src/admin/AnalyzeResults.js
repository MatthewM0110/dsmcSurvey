...
function AnalyzeResults() {
  ...
  const [respondentsFilter, setRespondentsFilter] = useState('All');
  ...
  useEffect(() => {
    const applyFiltersAndSort = () => {
      ...
      if (filterEndDate) {
        result = result.filter(survey => new Date(survey.end_date) <= new Date(filterEndDate));
      }
      ...
      if (sortType === 'mostCompleted') {
        result.sort((a, b) => b.totalCompleted - a.totalCompleted);
      } else if (sortType === 'leastCompleted') {
        result.sort((a, b) => a.totalCompleted - b.totalCompleted);
      }
    ...
    applyFiltersAndSort();
  }, [surveys, filterName, filterStartDate, filterEndDate, filterStatus, sortType]);
  ...
  const handleRespondentsFilterChange = (event) => {
    setRespondentsFilter(event.target.value);
  };
  ...
  const renderCompletionProgressBar = (completed, total) => {
    const completionPercentage = (completed / total) * 100;
    const responsePercentage = (completed / total) * 100; // Calculate response percentage
    return (
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Typography variant="body2">{`${completed}/${total}`}</Typography>
          <LinearProgress variant="determinate" value={completionPercentage} sx={{ width: '150px' }} />
        </Box>
        <Typography variant="body2">{`${responsePercentage.toFixed(2)}% Responded`}</Typography>
      </Box>
    );
  };
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4 }}>
      ...
      <Box sx={{ mb: 4, display: 'flex', gap: 2, alignItems: 'center' }}>
        ...
        <FormControl sx={{ minWidth: 200 }}>
          ...
          >
            ...
            <MenuItem value="mostCompleted">Most Completed</MenuItem>
            <MenuItem value="leastCompleted">Least Completed</MenuItem>
          </Select>
        </FormControl>
      </Box>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <Typography variant="body2" sx={{ color: theme.palette.primary.main, fontSize: '1rem' }}>
                      {survey.respondents ? renderCompletionProgressBar(survey.totalCompleted, survey.respondents.length) : 'Loading...'}
                    </Typography>
                  </Box>
                </Box>
              </CardActions>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2 }}>
            <FormControl>
              ...
              <Select
                value={respondentsFilter}
                onChange={handleRespondentsFilterChange}
              >
          ...
          <List>
            {respondents
              .filter(respondent => {
                if (respondentsFilter === 'Completed') {
                  return respondent.completed;
                } else if (respondentsFilter === 'NotCompleted') {
                  return !respondent.completed;
                }
                return true;
              })
        ...
        <DialogActions>
          <Button component={Link} to="/sendNotification" color="primary">
            Send Reminder
          </Button>
